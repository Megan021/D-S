...
const LoginDetailModel = ({ isOpen, toggleEditLogin }) => {
        ...
        <form className=''>
          ...
          <div>
            <button type='submit' className='flex items-center justify-between bg-black border border-black text-white px-4 py-2 rounded-sm w-full mb-3'>
              Save Changes<BsArrowRight className='text-2xl' />
            </button>
            <button type='button' className='flex items-center justify-between border border-black px-4 py-2 rounded-sm w-full' onClick={toggleEditLogin}>
              Cancel<BsArrowRight className='text-2xl' />
            </button>
          </div>
        </form>
      <button onClick={toggleEditLogin} className='absolute -top-3 -right-3 text-3xl bg-white rounded-full'><SlClose /></button>
      </motion.div>
    </div>
    </>
  )
